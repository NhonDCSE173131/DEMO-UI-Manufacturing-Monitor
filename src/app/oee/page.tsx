...
const OEEPage = () => {
  ...
  const oeeQuery = useMemo(
    () => toOeeQuery(oeeTrendRange, selectedShift, selectedAreaFilter, selectedStatusFilter),
    [oeeTrendRange, selectedShift, selectedAreaFilter, selectedStatusFilter],
  );
  ...
  const reasonMap =
    oeeAnalytics.losses.length > 0
      ? oeeAnalytics.losses.reduce((acc: Record<string, number>, item) => {
          const reason = String(item.label || item.key || item.name || item.id || 'OTHER');
          acc[reason] = (acc[reason] || 0) + Number(item.lossPercent ?? item.lossMinutes ?? item.value ?? 0);
          return acc;
        }, {})
      : scopedParetoEvents
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      {(machinesLoading || alarmsLoading || oeeAnalytics.loading || machinesError || alarmsError || oeeAnalytics.error) && (
        <div className="card-industrial p-3 text-xs border border-industrial-border/20 text-industrial-text-secondary">
          {machinesLoading || alarmsLoading || oeeAnalytics.loading
            ? (selectedLanguage === 'en' ? 'Loading OEE analytics from backend...' : 'Đang tải phân tích OEE từ backend...')
            : `${selectedLanguage === 'en' ? 'Backend unavailable. Showing local fallback.' : 'Backend tạm thời không phản hồi. Đang hiển thị dữ liệu dự phòng.'} ${machinesError || alarmsError || oeeAnalytics.error || ''}`}
        </div>
      )}
