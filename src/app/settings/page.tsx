...
const SettingsPage = () => {
  ...
  const { thresholds, loading: thresholdsLoading, error: thresholdsError } = useSettingsThresholds();
  const messages = selectedLanguage === 'en' ? enMessages : viMessages;
  const areaOptions = ['all', ...new Set(machines.map((machine) => machine.area))];

  return (
    <div className="space-y-6 animate-fade-in">
      {(machinesLoading || thresholdsLoading || machinesError || thresholdsError) && (
        <div className="card-industrial p-3 text-xs border border-industrial-border/20 text-industrial-text-secondary">
          {machinesLoading || thresholdsLoading
            ? (selectedLanguage === 'en' ? 'Loading system settings from backend...' : 'Đang tải cài đặt hệ thống từ backend...')
            : `${selectedLanguage === 'en' ? 'Backend unavailable. Showing local fallback.' : 'Backend tạm thời không phản hồi. Đang hiển thị dữ liệu dự phòng.'} ${machinesError || thresholdsError || ''}`}
        </div>
      )}
          ...
          <div className="border-t border-industrial-border/20 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              ...
              <div className="bg-industrial-darker/60 border border-industrial-border/20 rounded-lg p-3">
                ...
                <p className="text-industrial-text font-medium">{selectedLanguage === 'en' ? `Critical after ${thresholds.alarmEscalationMinutes}m unack` : `Nghiêm trọng sau ${thresholds.alarmEscalationMinutes} phút chưa xác nhận`}</p>
              </div>
              <div className="bg-industrial-darker/60 border border-industrial-border/20 rounded-lg p-3">
          ...
          <div className="border-t border-industrial-border/20 pt-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 text-sm">
              ...
              <div className="bg-industrial-darker/60 border border-industrial-border/20 rounded-lg p-3">
                ...
                <p className="text-industrial-text font-medium">{selectedLanguage === 'en' ? `${thresholds.retentionDays} days rolling data` : `Dữ liệu cuốn chiếu ${thresholds.retentionDays} ngày`}</p>
              </div>
            </div>
          </div>
