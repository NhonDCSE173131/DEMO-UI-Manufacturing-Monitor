...
export const mapApiMachineToUi = (input: Partial<Machine> & Record<string, unknown>): Machine => {
  ...
  const status = normalizeStatus(asString(input.status, input.machineState, input.connectionStatus));
  const mode = normalizeMode(asString(input.mode, input.operationMode));
  const oee = asNumber(input.oee, input.todayOee, input.oee) || 0;
  const availability = asNumber(input.availability, input.availabilityPct) || 0;
  const performance = asNumber(input.performance, input.performancePct) || 0;
  const quality = asNumber(input.quality, input.qualityPct) || 0;
  ...
  return {
    ...
    ngCount: asNumber(input.ngCount, input.rejectParts, input.badParts) || 0,
    ...
    toolLifeRemainingPct: asNumber(input.toolLifeRemainingPct, input.remainingToolLifePct) as number | undefined,
    spindleSpeedRpm: asNumber(input.spindleSpeedRpm, input.spindleRpm) as number | undefined,
    feedRateMmMin: asNumber(input.feedRateMmMin) as number | undefined,
    cuttingSpeedMMin: asNumber(input.cuttingSpeedMMin) as number | undefined,
    depthOfCutMm: asNumber(input.depthOfCutMm) as number | undefined,
    feedPerToothMm: asNumber(input.feedPerToothMm) as number | undefined,
    widthOfCutMm: asNumber(input.widthOfCutMm) as number | undefined,
    materialRemovalRateCm3Min: asNumber(input.materialRemovalRateCm3Min) as number | undefined,
    spindleLoadPct: asNumber(input.spindleLoadPct) as number | undefined,
    vibrationPct: asNumber(input.vibrationPct) as number | undefined,
    temperatureC: asNumber(input.temperatureC) as number | undefined,
    weldingCurrentA: asNumber(input.weldingCurrentA) as number | undefined,
    servoLoadPct: asNumber(input.servoLoadPct) as number | undefined,
    currentProgram: asString(input.currentProgram, input.programName) || 'N/A',
    area: asString(input.area, input.areaName, input.lineName) || 'Xuong chinh',
    rawTelemetry:
      (input.rawTelemetry as Machine['rawTelemetry']) || {
        ...
        vibrationPct: asNumber(input.vibrationPct),
